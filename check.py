...
# 生成一个列表
def add_list(job, error_info):
    return [job[0], job[1], error_info]
...
# 检查内容
def check_content(job_path, info_lst, job_info):
    # 检查非内容字段有无截断HTML标签
    ...
    cursor = conn.cursor()
    # 列出任务信息
    info_lst.append(["任务ID", "任务名", "错误信息"])
    info_lst.append(job_info)
    try:
        print('---------开始检查非内容字段是否有HTML标签--------')
        contents1 = cursor.execute(sql1)
        for job1 in contents1:
            info1 = add_list(job1, "标题或时间有HTML标签")
            info_lst.append(info1)
            # print('{}\t {}\t标题或时间有HTML标签'.format(job1[0], job1[1]))

        print('\n---------开始检查内容、标题、发布时间是否有空--------')
        contents2 = cursor.execute(sql2)
        for job2 in contents2:
            # info = add_list(job2,"内容或标题或发布时间有空值")
            info2 = [job2[0], job2[1], "有空值"]
            info_lst.append(info2)
            # print('{}\t {}\t内容或标题或发布时间有空值'.format(job2[0], job2[1]))

        print('\n---------开始检查内容字段是否有input标签--------')
        contents3 = cursor.execute(sql3)
        for job3 in contents3:
            info3 = add_list(job3, "内容有Input标签没有去除")
            info_lst.append(info3)
            # print('{}\t {}\t内容有input标签没有去除'.format(job3[0], job3[1]))
            ...
            if isinstance(content, str):
                ...
                if result != None:
                    if result.group(0).count('<') != result.group(0).count('>'):
                        info4 = add_list(job4, "内容可能有截断标签")
                        info_lst.append(info4)
        ...
        for job5 in contents5:
            ...
            if judge_value != 1:
                ...
                if judge_value != 1:
                    info5 = add_list(job5, "采集内容不是标讯(仅做参考)")
                    info_lst.append(info5)
                    # print('{}\t {}\t {tip}'.format(job5[0], job5[1], tip='采集内容不是标讯(仅做参考)'))
            else:
                pass
        print('\n---------检查标题、发布时间格式是否正确--------')
...
def get_jobs(siteid, db_path):
    # job_dic = OrderedDict()
    sql = '''SELECT j.jobid, j.jobname FROM "Job"j WHERE j.siteid = %s''' % siteid
    conn = connect_db(db_path)
    cursor = conn.cursor()
    try:
...
def check_db():
    ...
    while True:
        ...
        if select_check == 1:
            ...
            while True:
                print('id ', '---', 'jobname')
                for jobid, jobname in sorted(job_dic.items(), key=lambda x:x[0]):
                    print(jobid, '---', jobname)
                print('input a flag:\n1 --- check all \n0 --- exit \nid --- select one job\n')
                selectid = int(input('flag = '))
                if selectid == 1:
                    info_lst = []  # 存储错误信息的列表
                    for job_id, job_name in sorted(job_dic.items(), key=lambda x:x[0]):
                        print("\n--- 开始执行检查：", job_name, job_id, "\n")
                        job_path = j_path % job_id
                        job_info = [job_id, job_name]
                        check_content(job_path, info_lst, job_info)
                    wb = Workbook()
                    ws = wb.active
