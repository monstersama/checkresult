...
job_path = r'''D:\练习Hyt\火车采集器V8 2\Data\%s\SpiderResult.db3'''
...
def check_db():
    ...
    while True:
        select_check_type = int(input("输入1检查规则，2检查内容，0退出检查："))
        if select_check_type == 1:
            check_rule(rule_path)
            print('\n----rules check end----\n')
        elif select_check_type == 2:
            while True:
                single_or_batch = int(input('输入1检查单任务，2检查多任务，3返回上一步，0退出检查：'))
                if single_or_batch == 1:
                    while True:
                        jobid = 1  #初始化jobid
                        while jobid==True:
                            jobid = input('jobid = ')
                            job_path = job_path % jobid
